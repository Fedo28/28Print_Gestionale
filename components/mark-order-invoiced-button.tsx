...
function MarkOrderInvoicedSubmitButton({
  ...
  return (
    <button
      ...
      className={`button ghost invoice-status-button${isDone ? " done" : ""}${showLabel ? " with-label" : ""}`}
      disabled={pending}
      title={title}
      type="submit"
    >
      ...
      {!compact || showLabel ? <span>{pending ? "Aggiorno..." : isDone ? "Riporta da fatturare" : "Fattura"}</span> : null}
    </button>
  );
}
...
export function MarkOrderInvoicedButton({
  ...
  return (
    <form action={markOrderInvoicedAction} className={`invoice-status-control${compact ? " compact" : ""}`}>
      ...
      <MarkOrderInvoicedSubmitButton compact={compact} invoiceStatus={invoiceStatus} showLabel={showLabel} />
    </form>
  );
}
